...
lazy val `munit` = module
  ...
  .settings(libraryDependencies += "com.dimafeng" %% "testcontainers-scala-munit" % "0.39.6" % Test)
  .settings(libraryDependencies += "ch.qos.logback" % "logback-classic" % "1.2.5" % Test)
  .settings(libraryDependencies += "org.http4s" %% "http4s-async-http-client" % "0.23.1" % Test)
  .settings(libraryDependencies += "org.http4s" %% "http4s-client" % "0.23.1" % Test)
