...
lazy val `munit` = module
  ...
  .settings(libraryDependencies += "com.dimafeng" %% "testcontainers-scala-munit" % "0.40.1" % Test)
  .settings(libraryDependencies += "ch.qos.logback" % "logback-classic" % "1.2.10" % Test)
  .settings(libraryDependencies += "org.http4s" %% "http4s-async-http-client" % "0.23.10" % Test)
  .settings(libraryDependencies += "org.http4s" %% "http4s-client" % "0.23.10" % Test)
...
lazy val site = module
  ...
  .settings(laikaConfig := laikaConfig.value.withRawContent.withConfigValue("livereload", !sys.env.contains("CI")))
  .settings(laikaConfig := laikaConfig.value.withConfigValue("livereload", !sys.env.contains("CI")))
  .settings(laikaConfig := laikaConfig.value.withConfigValue("document.spanish", "false"))
