...
> Si no sabes como funciona la sintaxis de GitHub Actions, [aquí](https://docs.github.com/en/actions/quickstart) tienes toda la documentación necesaria para aprender a utilizarla (en inglés).
...
El primer paso consistirá en asegurarnos que la rama `develop` existe. Para ello vamos a usar la [CLI de GitHub](https://cli.github.com) (`gh`).
...
> `gh` nos permite hacer múltiples operaciones en GitHub desde la consola, de una forma sencilla y concisa. Si quieres saber más sobre los distintos comandos, te animo a que consultes [su documentación](https://cli.github.com/manual/) (en inglés).

El siguiente paso será hacer checkout de la rama `develop`. Debemos añadir `fetch-depth: 0` para evitar que el workflow haga un [`shallow-clone`](https://git-scm.com/docs/git-clone#Documentation/git-clone.txt---depthltdepthgt).
...
> Necesitamos establecer las opciones de `user.email` y `user.name` de Git puesto que estas [no se inicializan](https://github.com/actions/checkout/issues/13#issuecomment-724415212) cuando se usa la acción de checkout.
...
A partir de este punto, podemos empezar a recibir PR de Scala Steward a la rama `develop`. Únicamente necesitaremos que dichas PR se mergeen de forma automática. Para ello, como ya se mencionó en la primera sección, podemos usar apps como [Mergify](https://mergify.com), GitHub Actions como [esta](https://github.com/marketplace/actions/merge-dependency-update-prs) o habilitar el [auto-merge](https://docs.github.com/es/pull-requests/collaborating-with-pull-requests/incorporating-changes-from-a-pull-request/automatically-merging-a-pull-request) en dichas PR usando un nuevo workflow, lo dejo a tu elección.
...
- Activar una [protección de rama](https://docs.github.com/en/repositories/configuring-branches-and-merges-in-your-repository/defining-the-mergeability-of-pull-requests/about-protected-branches#require-status-checks-before-merging) sobre la rama `develop` para asegurar que las PR siempre se prueban encima de todas las actualizaciones que ya hayan tenido lugar.
